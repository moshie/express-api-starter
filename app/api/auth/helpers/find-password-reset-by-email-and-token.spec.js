...
describe('Find password reset by email and token', function () {
    ...
    it('should reject with a 500 response error', function () {
        ...
        return expect(findPasswordResetByEmailAndToken('example@test.com', 'token')).to.be.eventually.rejectedWith(ResponseException, errorMessage).and.have.property('statusCode', 500)
    })

    it('should reject with a 422 response error if no entry was found', function () {
        PasswordResets.findOne.yields(null, null)

        return expect(findPasswordResetByEmailAndToken('example@test.com', 'token')).to.be.eventually.rejectedWith(ResponseException, 'Invalid Token').and.have.property('statusCode', 422)
    })
